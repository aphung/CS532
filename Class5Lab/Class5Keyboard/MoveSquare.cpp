...
float yLocation = 200.0f; // Keep track of our position on the y axis.  
float xLocation = 200.0f; // Keep track of our position on the X axis.
...
void keyControl (void) {  
	...
	if (keyStates['b']) { // If 't' has been pressed rotate counter clockwise 
		if (Move) {  
			xScale += 0.01f;
			yScale += 0.01f;
		}
	}
	if (keyStates['s']) { // If 't' has been pressed rotate counter clockwise 
		if (Move) {  
			xScale -= 0.01f;
			yScale -= 0.01f;
		}
...
void display() {
	...
	double X [4] = {-50, 50, 50, -50}; // x vertices coordinates
	double Y [4] = {-50, -50, 50, 50}; // y vertices coordinates
	double Xn[4] = {0, 0, 0, 0};
	double Yn[4] = {0, 0, 0, 0};
	...
	if (Move) {
		...
		*--------------------------------------------------------------------------------------*/
		//
		//double a = cos(zRotationAngle); // rot/scale/reflection
		//double b = -sin(zRotationAngle);
		//double c = sin(zRotationAngle);
		//double d = cos(zRotationAngle);
		//int tx = 100 + xLocation;
		//int ty = 100 + yLocation;
